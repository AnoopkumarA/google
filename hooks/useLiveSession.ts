...
export const useLiveSession = ({ context }: UseLiveSessionProps) => {
  ...
  const connect = async () => {
    ...
    try {
      ...
      // If session closed immediately during connect handshake
      if (!isSessionActive.current) {
         // It might be open now if onopen fired synchronously or microtasked
         // But usually onopen fires after connect resolves.
         // Let's assume it's open if no error was thrown.
         isSessionActive.current = true;
      }
  ...
  const startAudioStreaming = () => {
    ...
    processorRef.current.onaudioprocess = (e) => {
      ...
      const now = Date.now();
      if (now - lastVolumeTimeRef.current > 50) { // Update max every 50ms (20fps)
          let sum = 0;
      ...
      // STRICT CHECK: Do not attempt to send if session is not active
      ...
      try {
        ...
        // Suppress expected error when socket closes mid-stream
        if (err.message && (err.message.includes("Closing") || err.message.includes("Closed"))) {
            console.warn("Socket closed while sending audio (suppressed)");
            isSessionActive.current = false;
        } else {
